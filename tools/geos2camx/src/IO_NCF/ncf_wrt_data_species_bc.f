c**** NCF_WRT_DATA_SPECIES_BC
c
      subroutine ncf_wrt_data_species_bc(ierr,action,iounit,this_hour,
     &              num_cols,num_rows,num_lays_in,num_lays_out,nspcs,
     &                                      spec_name,height,concbc)
      implicit none
c
c-----------------------------------------------------------------------
c    Description:
...
c           concbc       R grdded array of concentrations
c       Outputs:
c           ierr         I error code
c
c-----------------------------------------------------------------------
...
c
      ...
      real          concbc(num_cols,num_rows,num_lays_in,nspcs)
c
c-----------------------------------------------------------------------
...
c
         ...
         do k=1,num_lays_out
            do j=1,num_rows
              array_3d(1,j,k) = concbc(1,j,k,ispc)
              array_3d(num_cols,j,k) = concbc(num_cols,j,k,ispc)
            enddo
            do i=1,num_cols
              array_3d(i,1,k) = concbc(i,1,k,ispc)
              array_3d(i,num_rows,k) = concbc(i,num_rows,k,ispc)
            enddo
         enddo
c
