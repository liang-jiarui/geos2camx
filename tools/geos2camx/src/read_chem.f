c234567
      ...
      SUBROUTINE READ_CHEM( N_CAMX_SPEC, N_GEOS_SPEC, ID_CAMX,
     &           GA_CAMX, CAMX_SPEC_MW,  
     &           ID_GEOS, MAP_IDX, MAP_FAC, camx_spec_names,
     &           geos_spec_names, lgas, e6unit,infile_name)

!
!******************************************************************************
!  Subroutine READ_CHEM reads species lists for input (GEOS-CHEM) and output
!  (CAMx) as well as mapping mechanism between them. 
!
!  Arguments as Output:
!  ============================================================================
!  (1) N_CAMX_SPEC   (INTEGER): NUMBER OF CAMX SPECIES 
!  (2) N_GEOS_SPEC   (INTEGER): NUMBER OF GEOS-CHEM SPECIES
!  (3) ID_CAMX       (INTEGER): ID FOR CAMX SPECIES
!  (4) GA_CAMX     (CHARACTER): TAG FOR GAS/AEROSOL 
!  (5) CAMX_SPEC_MW   (REAL*4): MOLECULAR WEIGHT OF CAMX SPECIES
!  (6) ID_GEOS       (INTEGER): ID FOR GEOS-CHEM SPECIES
!  (7) MAP_IDX       (INTEGER): GEOS-CHEM ID FOR MAPPING EACH CAMX SPECIES 
!  (8) MAP_FAC        (REAL*4): FRACTION FOR MAPPING EACH CAMX SPECIES
!
!  NOTES:
!******************************************************************************
!     
...
c      INCLUDE 'FILES_GEOSCHEM.EXT'

      integer, intent(in)  :: e6unit
      INTEGER, INTENT(OUT) :: N_CAMX_SPEC, N_GEOS_SPEC
      INTEGER, INTENT(OUT) :: ID_CAMX(500), ID_GEOS(500)
      INTEGER, INTENT(OUT) :: MAP_IDX(500,99) ! bkoo (3/25/09)
      REAL*4,  INTENT(OUT) :: MAP_FAC(500,99) ! bkoo (3/25/09)
      REAL*4,  INTENT(OUT) :: CAMX_SPEC_MW(500)
      CHARACTER(LEN=1), INTENT(OUT) :: GA_CAMX(500)
      character*256 infile_name
      ...
      REAL*4              :: FAC, S_MW
      CHARACTER (LEN=12)  :: CAMX_SPEC_NAMES(500)
      CHARACTER (LEN=12)  :: GEOS_SPEC_NAMES(500)
      CHARACTER (LEN=12)  :: TMP1, TMP2
      logical             :: lgas(*)
      ...
      INTEGER             :: FIND_INDEX, CAMX_INDEX, GEOS_INDEX
      ...
      DO
         ...
         IF ( IOS .NE. 0 ) THEN
              write(*,'(/,A)' ) 'ERROR in READ_CHEM.'
              write(*,'(A,I5)' ) 'Cannot read file at line: ',numrecs
              write(*,'(2A,//)' ) 'File: ',TRIM(infile_name)
              stop
         ENDIF
     ...
     &                STR,  N,  LINE2,     S_MW
         ...
         IF ( IOS > 0 ) THEN
              write(*,'(/,A)' ) 'ERROR in READ_CHEM.'
              write(*,'(A,I5)' ) 'Cannot read CAMx species data at line: ',numrecs
              write(*,'(2A)' ) 'File: ',TRIM(infile_name)
              write(*,'(A,/,A,//)' ) 'Line read: ',TRIM(line)
              stop
         ENDIF

         N_CAMX_SPEC = N_CAMX_SPEC + 1
         GA_CAMX( N_CAMX_SPEC )         =  STR
         if( GA_CAMX( N_CAMX_SPEC )(1:1) .EQ. 'G') lgas(N_CAMX_SPEC) = .true.
         if( GA_CAMX( N_CAMX_SPEC )(1:1) .EQ. 'A') lgas(N_CAMX_SPEC) = .false.

         ID_CAMX( N_CAMX_SPEC )         =  N
         CAMX_SPEC_NAMES( N_CAMX_SPEC ) = LINE2
         CAMX_SPEC_MW( N_CAMX_SPEC )    = S_MW
      ...
      DO
         ...
         IF ( IOS > 0 ) THEN
              write(*,'(/,A)' ) 'ERROR in READ_CHEM.'
              write(*,'(A,I5)' ) 'Cannot read GeosChem species data at line: ',numrecs
              write(*,'(2A)' ) 'File: ',TRIM(infile_name)
              write(*,'(A,/,A,//)' ) 'Line read: ',TRIM(line)
              stop
         ENDIF
      ...
      DO
         ...
         IF ( IOS > 0 ) THEN
              write(*,'(/,A)' ) 'ERROR in READ_CHEM.'
              write(*,'(A,I5)' ) 'Cannot read mapping data at line: ',numrecs
              write(*,'(2A)' ) 'File: ',TRIM(infile_name)
              write(*,'(A,/,A,//)' ) 'Line read: ',TRIM(line)
              stop
         ENDIF
         CAMX_INDEX = find_index(line1, CAMX_SPEC_NAMES(1:N_CAMX_SPEC),
     &                           N_CAMX_SPEC)
         ...
         DO WHILE ( IFLAG1 == 0 )
            ...
            READ( WORD, '(f6.3, 1x, a8)', IOSTAT=IOS ) FAC, TMP2

            GEOS_INDEX = find_index(TMP2, 
     &                    GEOS_SPEC_NAMES(1:N_GEOS_SPEC), N_GEOS_SPEC)

            MAP_IDX(CAMX_INDEX, ICOUNT) = GEOS_INDEX
            MAP_FAC(CAMX_INDEX, ICOUNT) = FAC
            ICOUNT = ICOUNT + 1
...
!------------------------------------------------------------------------------
      ...
      IF ( LEN_TRIM( PATTERN ) /= LEN_TRIM( REPLTXT ) ) THEN 
         WRITE( 6, '(a)' ) 'ERROR in STRREPL.'
         WRITE( 6, '(a)' ) REPEAT( '=', 79 )
          write(*,'(/,A)' ) 'ERROR in READ_CHEM.'
          write(*,'(A,I5)' ) 'PATTERN and REPLTXT must have same # of characters!'
          stop
      ENDIF
